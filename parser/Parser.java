...
public class Parser {
  ...
  private EnumSet<Tokens> relationalOps = EnumSet.of(Tokens.Equal, Tokens.NotEqual, Tokens.Less, Tokens.LessEqual,
      Tokens.GreaterEqual, Tokens.Greater);
   ...
   *
   * @return block tree
   * @exception SyntaxError - thrown for any syntax error e.g. an expected left
   *                        brace isn't found
   */
  ...
  /**
   * < pre> d -> type name ==> decl -> type name funcHead block ==> functionDecl
   * </pre>
   *
  ...
  /**
   * < pre> funHead -> '(' (decl list ',')? ')' ==> formals note a funhead is a
  ...
  /**
   * < pre> S -> 'if' e 'then' block 'else' block ==> if -> 'while' e block ==>
   * while -> 'return' e ==> return -> block -> name '=' e ==> assign
   * </pre>
   *
  ...
  /**
   * < pre> se -> t -> se '+' t ==> + -> se '-' t ==> - -> se '|' t ==> or This
  ...
  /**
   * < pre> t -> f -> t '*' f ==> * -> t '/' f ==> / -> t '&' f ==> and This rule
   * indicates we should pick up as many <i>f</i>'s as possible; the <i>f</i>'s
  ...
  /**
   * < pre> f -> '(' e ')' -> name -> <int> -> name '(' (e list ',')? ')' ==> call
   * </pre>
   *
  ...
  public AST rFactor() throws SyntaxError {
    ...
    if (isNextTok(Tokens.DateLit)) {
      t = new DateLitTree(currentToken);
      scan();
      return t;
    }
    if (isNextTok(Tokens.NumberLit)) {
      t = new NumberLitTree(currentToken);
      scan();
      return t;
    }
    t = rName();
